<div class="min-h-screen bg-slate-950 text-slate-300 selection:bg-accent selection:text-white" x-data="{ scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 50)" x-init="window.addEventListener('redirect-to', e => { window.open(e.detail.url, '_blank'); });">
    ...
    <main class="py-8 max-w-7xl mx-auto px-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            ...
            <!-- Left Column: Image -->
            <div class="bg-slate-900 border border-slate-800/80 rounded-xl p-4 flex items-center justify-center relative shadow-lg shadow-black/25">
                <div class="w-full aspect-[4/3] relative flex items-center justify-center max-h-[350px]">
                    @if ($product->getFirstMediaUrl('image'))
                        <img src="{{ $product->getFirstMediaUrl('image') }}" alt="{{ $product->name }}" class="max-w-full max-h-full object-contain drop-shadow-lg" />
                    @else
                        <x-lucide-shopping-bag class="w-16 h-16 text-slate-800" />
                    @endif
                </div>
            </div>
            ...
            <div class="flex flex-col space-y-6">
                <div>
                    ...
                    <h1 class="text-2xl md:text-3xl font-black text-white mt-1 leading-snug">{{ $product->name }}</h1>
                    
                    <div class="text-xl md:text-2xl font-black text-accent mt-3">
                        @if ($product->price)
                            Rp {{ number_format($product->price, 0, ',', '.') }}
                        @else
                            <span class="text-sm font-bold text-slate-400 uppercase tracking-wider">Hubungi Kami</span>
                        @endif
                    </div>
                </div>
                
                @if($product->short_description)
                    <p class="text-slate-400 text-sm leading-relaxed">{{ $product->short_description }}</p>
                @endif
        ...
        <div class="mt-12 border-t border-slate-900 pt-8 space-y-8">
            @if($product->description)
                <div class="prose prose-sm prose-invert max-w-none">
                    <h3 class="text-base font-bold text-white border-b border-slate-800 pb-2 mb-4">Deskripsi Lengkap</h3>
                    <div class="whitespace-pre-line text-slate-400 leading-relaxed text-sm">{{ $product->description }}</div>
                </div>
            @endif
            
            @if($product->specifications && is_array($product->specifications) && count($product->specifications) > 0)
                <div>
                    <h3 class="text-base font-bold text-white border-b border-slate-800 pb-2 mb-4">Spesifikasi</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        @foreach($product->specifications as $key => $val)
                            <div class="flex items-center justify-between p-3.5 bg-slate-900/60 rounded-lg border border-slate-800/80">
                                <span class="font-bold text-slate-400 uppercase text-[10px] tracking-wider">{{ $key }}</span>
                                <span class="text-white text-xs">{{ $val }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
