<div class="min-h-screen bg-slate-950 text-slate-300 selection:bg-accent selection:text-white" x-data="{ scrolled: false, activeCategory: 'all' }" @scroll.window="scrolled = (window.pageYOffset > 20)" x-init="window.addEventListener('redirect-to', e => { window.open(e.detail.url, '_blank'); });">
    ...
    <main class="py-8 max-w-7xl mx-auto px-6">
        ...
        <div class="mb-10 flex flex-col md:flex-row md:items-end md:justify-between gap-4 border-b border-slate-900 pb-6">
            <div>
                <h1 class="text-2xl md:text-3xl font-black text-white tracking-tight">Semua Produk</h1>
                <p class="text-slate-400 text-xs md:text-sm mt-1 max-w-xl">Temukan solusi perangkat keras dan lunak terbaik untuk bisnis & infrastruktur teknologi Anda.</p>
            </div>
        </div>
                ...
                <div class="bg-slate-900/60 border border-slate-800/80 rounded-xl p-5 sticky top-24">
                    ...
                    <div class="space-y-1">
                        <button @click="activeCategory = 'all'" 
                            ...
                            <span class="px-1.5 py-0.5 rounded bg-slate-800 text-[10px]">{{ $products->count() }}</span>
                        </button>
                        ...
                        <div x-show="activeCategory === 'all' || activeCategory === '{{ $product->category?->slug }}'" 
                            ...
                            <div class="flex items-center justify-between mb-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                                ...
                                @if($product->is_featured)
                                    <span class="text-accent text-[9px] bg-accent/10 border border-accent/20 rounded px-1.5 py-0.5">Unggulan</span>
                                @endif
                            </div>

                            <!-- Product Image -->
                            <div class="w-full h-32 md:h-40 relative flex items-center justify-center rounded-lg bg-slate-950 overflow-hidden p-3 mb-4 border border-slate-800/40">
                                @if ($product->getFirstMediaUrl('image'))
                                    <img src="{{ $product->getFirstMediaUrl('image') }}" alt="{{ $product->name }}" class="max-w-full max-h-full object-contain group-hover:scale-105 transition-transform duration-500 drop-shadow-md" />
                                @else
                                    <x-lucide-shopping-bag class="w-8 h-8 text-slate-800" />
                                @endif
                            </div>
                            ...
                            <div class="pt-3 border-t border-slate-800/60 flex items-center justify-between gap-3 mt-auto">
                                <div class="flex flex-col">
                                    @if ($product->price)
                                        <span class="text-xs md:text-sm font-extrabold text-white">Rp {{ number_format($product->price, 0, ',', '.') }}</span>
                                    @else
                                        <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Hubungi Kami</span>
                                    @endif
                                </div>
                                <a href="{{ route('products.show', $product->slug) }}" class="flex items-center gap-1 px-3 py-1.5 bg-accent hover:bg-accent/90 text-white font-extrabold text-[10px] uppercase tracking-wider rounded-lg transition-all">
                                    <span>Detail</span>
                    ...
                    @empty
                        <div class="col-span-full py-16 text-center">
                            <div class="w-16 h-16 bg-slate-900 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-800">
                                <x-lucide-package-x class="w-8 h-8 text-slate-500" />
                            </div>
                            <h3 class="text-sm font-bold text-white">Belum Ada Produk</h3>
                        </div>
                    @endforelse
                </div>
            </section>
