...
>
    ...
    <!-- 1. Navbar -->
    <header 
        :class="scrolled ? 'bg-slate-950/80 backdrop-blur-xl border-b border-slate-800/40 shadow-lg shadow-black/10 py-3.5' : 'bg-transparent py-6'"
    ...
    >
        <div class="max-w-7xl mx-auto px-6 flex items-center justify-between">
            ...
            <a href="#" class="flex items-center gap-2 font-extrabold text-xl tracking-wider text-white transition-transform duration-300 hover:scale-[1.02]">
                @if ($settings['logo'])
                    <img src="{{ $settings['logo'] }}" alt="{{ $settings['company_name'] }}" class="h-10 max-w-[170px] object-contain" />
                @else
                    <x-lucide-terminal class="w-6 h-6 text-accent" />
                    <span>{{ strtoupper(explode(' ', $settings['company_name'])[0]) }}</span>
                @endif
            </a>
            ...
            <div class="hidden md:flex items-center gap-4">
                <a href="#contact" class="px-5 py-2.5 bg-gradient-to-r from-accent to-indigo-600 hover:from-accent/90 hover:to-indigo-600/90 text-white font-bold text-xs rounded-lg transition-all duration-300 shadow-md shadow-accent/10 hover:shadow-accent/20 hover:scale-[1.03]">
                    Hubungi Kami
                </a>
            </div>
            ...
            <div class="md:hidden" x-data="{ open: false }">
                ...
                <div 
                    ...
                    style="display: none;"
                >
                    <a @click="open = false" href="#" class="block text-slate-300 hover:text-white text-sm font-semibold py-2">Home</a>
                    <a @click="open = false" href="#about" class="block text-slate-300 hover:text-white text-sm font-semibold py-2">Tentang Kami</a>
                    <a @click="open = false" href="#services" class="block text-slate-300 hover:text-white text-sm font-semibold py-2">Layanan</a>
                    <a @click="open = false" href="#products" class="block text-slate-300 hover:text-white text-sm font-semibold py-2">Produk</a>
                    <a @click="open = false" href="#team" class="block text-slate-300 hover:text-white text-sm font-semibold py-2">Tim</a>
                    <a @click="open = false" href="#faq" class="block text-slate-300 hover:text-white text-sm font-semibold py-2">FAQ</a>
                    <a @click="open = false" href="#contact" class="block w-full text-center px-4 py-2.5 bg-accent text-white text-xs font-bold rounded-lg hover:bg-accent/90 transition-colors">Hubungi Kami</a>
                </div>
            </div>
        </div>
    </header>
